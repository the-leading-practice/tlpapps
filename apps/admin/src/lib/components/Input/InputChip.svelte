<script lang="ts" context="module">
	...
	import { DynamicTransition } from '$lib/transitions/Transition.js';
...
>
	import type { DataListItem } from '../DataList/types.js';
	...
	// Event Dispatcher
	type InputChipEvent = {
		add: { event: SubmitEvent | MouseEvent; chipIndex: number; chipValue: string };
		remove: { event: MouseEvent; chipIndex: number; chipValue: string };
		invalid: { event: SubmitEvent | MouseEvent; input: string };
	};
  ...
  export let itemList: DataListItem[];
	...
	// Classes
	const cBase = 'input-bordered rounded textarea cursor-pointer mb-4';
  const chipsBase = 'px-[8px] rounded';
	const cChipWrapper = 'space-y-4';
	const cChipList = 'flex flex-wrap gap-2';
	const cInputField = 'unstyled bg-transparent border-0 !ring-0 p-2 w-full';
  ...
  // keep track of the values already added
  $: if( chipValues.length !== listMask.length ) {
    listMask = [];
    // add new chips
    chipValues.forEach( (val) => {
      if( !listMask.includes( val.val ) ) listMask.push( val.val ) 
    } );
