...
const createController = () => {
  ...
  const oauth = async( req: express.Request, res: express.Response ) => {
    ...
    // get token
    ghlTokenService.getAccessToken( ghlCode as string )
    .then( async ( msg ) => {
      ...
      let calendar = existingToken ? existingToken.calendar : 'need-calendar-id';
      let timezone = existingToken ? existingToken.timezone : '';
      let pushGHL = existingToken && existingToken.pushGHL ? existingToken.pushGHL : false;
      let pushAppt = existingToken && existingToken.pushAppt ? existingToken.pushAppt : false;
      let pushPat = existingToken && existingToken.pushPat ? existingToken.pushPat : false;
      let software = existingToken && existingToken.software ? existingToken.software : 'unknown';
