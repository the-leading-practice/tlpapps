/**
 * Patient controller.
 * Ported from patient-service/src/controllers/patientController.ts.
 */
...
const createController = () => {
	...
	const patients = async (req: express.Request, res: express.Response) => {
		...
		if (loc.location) {
			const ret = await patientDataService.getPatients(loc.location);
			res.type('json');
	...
	const createPatient = async (req: express.Request, res: express.Response) => {
		...
		for (const patient of reqData.patients) {
			...
			if (!pushGHL || !pushPat) {
				logger.writeLog('warn', `no push header found - eating data`);
			...
			if (!patient.patientId) {
				logger.writeLog('warn', `bad request - no patientId`);
				resp.fail.push({ index: idx, status: 4, message: 'bad request - no patientId' });
			...
			if (!mapping) {
				...
				if (contacts && Array.isArray(contacts.data) && contacts.data.length > 0) {
					contacts.data.forEach((contact: any) => {
						if (verifyPatient(patient, contact)) {
							logger.writeLog(
								'debug',
								`found verified ${patient.patientId} contact ${contact.contactId}`,
							);
				...
				if (data.status === 200) {
					...
					logger.writeLog(
						'debug',
						`found GHL contact ${verifiedContact.firstName} ${verifiedContact.lastName}`,
					);
			...
			if (verifiedContact) {
				...
				logger.writeLog(
					'debug',
					`verified contact found - checking data to see if update is required ${patient.patientId} to ${mapping.contactId}`,
				);
			...
			if (mapping && mapping.contactId.length === 0) {
				...
				if (contactResp.status >= 200 && contactResp.status < 300) {
					// successful add to GHL
					logger.writeLog(
						'debug',
						`${patient.patientId} new ghl contactid ${contactResp.data.contact.id}`,
					);
			...
			// existing patient
				...
				if (needUpdate) {
					...
					if (contactResp.status >= 200 && contactResp.status < 300) {
						logger.writeLog(
							'debug',
							`${patient.patientId} ghl contact updated contactId: ${mapping.contactId}`,
						);
						resp.success.push({ id: patient.patientId });
					} else {
						resp.fail.push({
						...
						logger.writeLog(
							'warn',
							`error: code ${contactResp.status} response ${contactResp.data}`,
						);
						logger.writeLog('warn', logger.addSlashes(JSON.stringify(patient)));
					}
				} else {
